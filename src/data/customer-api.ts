...
export async function downloadAccountDocument(kind: 'appointments' | 'orders' | 'estimates' | 'sales' | 'receipts' | 'cash-receipts', id: string) {
  ...
  link.download = `kut-shoppe-${kind === 'appointments' ? 'appointment' : kind === 'orders' ? 'order' : kind.endsWith('receipts') ? 'receipt' : 'estimate'}.html`;
  document.body.append(link); link.click(); link.remove();
  window.setTimeout(() => URL.revokeObjectURL(objectUrl), 1000);
}
