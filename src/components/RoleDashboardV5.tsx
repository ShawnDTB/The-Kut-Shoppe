...
function IdentityChange({ account, channel, currentValue, onVerified }: { account: PlatformAccount; channel: 'email' | 'phone'; currentValue: string; onVerified: () => void }) {
  ...
  const label = channel === 'email' ? 'Email address' : 'Mobile phone';
  ...
  const request = () => {
    try {
      ...
      setChallenge(next);
      setCode('');
      setMessage(`Verification code sent. Enter it below to finish updating your ${channel}.`);
      setError('');
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'Unable to start the update.');
      setMessage('');
    }
  };
  ...
  const verify = () => {
    try {
      ...
      setError(verificationError instanceof Error ? verificationError.message : 'Unable to verify the update.');
    }
  };

  return <section className="v5-identity-change"><label>{label}<input type={channel === 'email' ? 'email' : 'tel'} autoComplete={channel === 'email' ? 'email' : 'tel'} value={value} onChange={(event) => { setValue(event.target.value); setMessage(''); setError(''); }} /></label><div className="v5-inline-actions"><button className="button button-secondary" type="button" disabled={!value.trim() || value === currentValue} onClick={request}>Update</button>{challenge ? <><input className="v5-code-input" aria-label={`${channel} verification code`} inputMode="numeric" maxLength={6} placeholder="6-digit code" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} /><button className="button" type="button" disabled={code.length !== 6} onClick={verify}>Confirm update</button></> : null}</div>{import.meta.env.DEV && challenge ? <small>Local verification code: {challenge.code}</small> : null}{message ? <p className="success-message" role="status">{message}</p> : null}{error ? <p className="form-error" role="alert">{error}</p> : null}</section>;
}

function CustomerDashboard({ initialAccount }: { initialAccount: PlatformAccount }) {
  const initialOrderId = typeof window === 'undefined' ? '' : new URLSearchParams(window.location.search).get('order') ?? '';
  ...
  useEffect(() => {
    const refresh = () => {
      const next = getPlatformSessionAccount();
      if (next) { setAccount(next); setProfile(getCustomerProfileV5(next)); }
  ...
  const refreshIdentity = () => {
    ...
    setAccount(updated);
    setProfile(getCustomerProfileV5(updated));
  };
  ...
  const canChange = (appointment: PlatformAppointment) => appointmentDate(appointment).getTime() - currentTime > noticeHours(appointment) * 60 * 60 * 1000;

  const cancelAppointment = (appointment: PlatformAppointment) => {
    const policyHours = noticeHours(appointment);
    if (!canChange(appointment)) {
      setAppointmentMessage(`Online cancellation closed ${formatNoticeWindow(policyHours)}. Call ${business.phone} for help.`);
      return;
    }
    const confirmed = window.confirm(`Cancel ${appointment.serviceName} on ${formatDate(appointment.date)} at ${appointment.time}? This action cannot be undone online.`);
    if (!confirmed) return;
    updateAppointment(appointment.id, { status: 'cancelled' });
    setAppointments(readAppointments());
    setAppointmentMessage('The appointment request was cancelled.');
  };
  ...
  const saveProfile = () => {
    try {
      ...
      setAccount(updated);
      setProfile(getCustomerProfileV5(updated));
      setProfileMessage('Profile and delivery address saved.');
      setProfileError('');
    } catch (error) {
      setProfileError(error instanceof Error ? error.message : 'Unable to save the profile.');
      setProfileMessage('');
    }
  };
  ...
  return <section className="section v5-customer-account platform-pattern platform-pattern-account"><div className="container route-wide"><header className="v5-account-header"><h1>Welcome, {account.name.split(/\s+/)[0]}.</h1><button className="button button-secondary" type="button" onClick={logout}>Log out</button></header>
    <section className="v5-account-section v5-appointments"><div className="v5-section-heading"><div><p className="eyebrow">Appointments</p><h2>Your appointments</h2></div><a className="button" href="/book">Book now</a></div>{appointmentMessage ? <p className="success-message" role="status">{appointmentMessage}</p> : null}{activeAppointments.length ? <div className="v5-appointment-list">{activeAppointments.map((appointment) => { const policyHours = noticeHours(appointment); const online = canChange(appointment); return <article key={appointment.id}><div><span className={`staff-status staff-status-${appointment.status}`}>{appointment.status.replaceAll('-', ' ')}</span><h3>{appointment.serviceName}</h3><dl><div><dt>Date</dt><dd>{formatDate(appointment.date)}</dd></div><div><dt>Time</dt><dd>{appointment.time}</dd></div><div><dt>Barber</dt><dd>{appointment.barberName.replace(/\.$/, '')}</dd></div></dl></div><div className="v5-appointment-policy"><p className="v5-account-status-note">Online changes close {formatNoticeWindow(policyHours)}.</p>{appointment.status === 'reschedule-proposed' && appointment.proposedDate ? <><strong>Proposed: {formatDate(appointment.proposedDate)} at {appointment.proposedTime}</strong><div className="v5-inline-actions"><button className="button" type="button" onClick={() => { respondToAppointmentProposal(appointment.id, true); setAppointments(readAppointments()); }}>Accept</button><button className="button button-secondary" type="button" onClick={() => { respondToAppointmentProposal(appointment.id, false); setAppointments(readAppointments()); }}>Decline</button></div></> : online ? <><div className="v5-inline-actions"><a className="button button-secondary" href={`/book?appointment=${appointment.id}`}>Change appointment</a><button className="text-button danger" type="button" onClick={() => cancelAppointment(appointment)}>Cancel appointment</button></div><small className="v5-confirm-note">Changes and cancellations remain subject to the assigned professional’s policy window.</small></> : <a className="text-link" href={business.phoneHref}>Call {business.phone}</a>}</div></article>; })}</div> : <p>No upcoming appointment requests.</p>}</section>
    <div className="v5-account-grid"><section className="v5-account-section"><div className="v5-section-heading"><div><p className="eyebrow">Orders</p><h2>Your orders</h2></div><a className="text-link" href="/shop">Shop</a></div>{customerOrders.length ? <div className="v5-order-list">{customerOrders.slice().reverse().map((order: StoreOrder) => <article key={order.id}><div><strong>{order.items.map((item) => `${item.quantity} × ${item.name}`).join(', ')}</strong><small>{order.status.replaceAll('-', ' ')} · {order.fulfillment}</small></div><strong>{formatMoney(order.totalCents)}</strong><button className="button button-secondary" type="button" onClick={() => setSelectedOrderId(order.id)}>View order</button></article>)}</div> : <div className="v5-account-recommendations"><strong>Nothing ordered yet.</strong><a href="/shop">Browse the Shop →</a><a href="/gallery">View recent work →</a><a href="/services">See services →</a></div>}</section>
      <section className="v5-account-section v5-profile-section"><p className="eyebrow">Profile</p><h2>Contact and delivery details</h2><div className="v5-profile-groups"><section className="v5-profile-group"><h3>Personal details</h3><div className="v5-profile-fields-v6"><label className="v5-profile-wide">Full name<input autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} /></label></div></section><section className="v5-profile-group"><h3>Contact methods</h3><div className="v5-contact-grid"><IdentityChange account={account} channel="email" currentValue={account.email} onVerified={refreshIdentity} /><IdentityChange account={account} channel="phone" currentValue={profile?.phone ?? ''} onVerified={refreshIdentity} /></div></section><section className="v5-profile-group"><h3>Delivery address</h3><div className="v5-profile-fields-v6"><label className="v5-profile-wide">Street address<input autoComplete="street-address" placeholder="Start typing your street address" value={address.line1} onChange={(event) => setAddress({ ...address, line1: event.target.value })} /></label><label className="v5-profile-wide"><span className="sr-only">Apartment, suite, unit, or P.O. box</span><input autoComplete="address-line2" placeholder="Apt., suite, unit, or P.O. box (optional)" value={address.line2} onChange={(event) => setAddress({ ...address, line2: event.target.value })} /></label><label>City<input autoComplete="address-level2" value={address.city} onChange={(event) => setAddress({ ...address, city: event.target.value })} /></label><label>State<input autoComplete="address-level1" value={address.state} onChange={(event) => setAddress({ ...address, state: event.target.value })} /></label><label>ZIP code<input autoComplete="postal-code" value={address.postalCode} onChange={(event) => setAddress({ ...address, postalCode: event.target.value })} /></label></div><small className="address-suggestion-status">Browser autofill is available. Google suggestions appear when the production Places key is configured.</small></section></div>{profileMessage ? <p className="success-message" role="status">{profileMessage}</p> : null}{profileError ? <p className="form-error" role="alert">{profileError}</p> : null}<button className="button" type="button" onClick={saveProfile}>Save profile</button></section></div>
    {selectedOrder ? <div className="v5-account-order-detail"><OrderReceiptV5 order={selectedOrder} products={products} accountMode onClose={() => { setSelectedOrderId(''); if (typeof window !== 'undefined') window.history.replaceState({}, '', '/account'); }} /></div> : null}
