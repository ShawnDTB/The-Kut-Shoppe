...
function buildCalendarDays(days = 49): CalendarDay[] {
  ...
  return Array.from({ length: days }, (_, offset) => {
    ...
    return { key, weekday: weekday.format(date), month: month.format(date), day: String(date.getDate()), isToday: key === todayKey(), isClosed: date.getDay() === 0 };
  });
}
...
function availableBarbersForSlot(input: {
  ...
  const eligible = getEligibleBarbers(input.service.id, input.profiles);
  return eligible.filter((barber) => {
    const id = barberId(barber);
    if (input.requestedBarberId !== 'any' && id !== input.requestedBarberId && barber.id !== input.requestedBarberId) return false;
    if (input.requestedBarberId === 'any' && !(barber.profile?.bookingRules.allowAnyAvailable ?? true)) return false;
...
export function BookingV5() {
  ...
  const editingDayIndex = editingAppointment ? Math.max(0, calendarDays.findIndex((day) => day.key === editingAppointment.date)) : 0;
  ...
  const [customer, setCustomer] = useState<CustomerDetails>({ name: editingAppointment?.customerName ?? account?.name ?? '', email: editingAppointment?.customerEmail ?? account?.email ?? '', phone: editingAppointment?.customerPhone ?? profile?.phone ?? '', note: editingAppointment?.customerNote ?? '' });
  ...
  const slotsForDate = (value: string) => service ? visibleTodaySlots(getAvailableTimeSlots({ service, requestedBarberId: selectedBarberId, date: value, profiles: staffProfiles, appointments: editingAppointment ? appointments.filter((item) => item.id !== editingAppointment.id) : appointments }), value) : [];
  ...
  useEffect(() => subscribeToAppointmentChanges(() => { setStaffProfiles(readStaffProfiles()); setAppointments(readAppointments()); }), []);
  ...
  if (appointmentId && !ownsAppointment) return <section className="section booking-v2-entry platform-pattern platform-pattern-booking"><div className="container narrow-container"><div className="booking-v2-confirmation"><p className="eyebrow">Account required</p><h1>This appointment cannot be changed here.</h1><p>Sign in with the customer account attached to the appointment, or call the shop for help.</p><div className="booking-v2-complete-actions"><a className="button" href="/account">Account / Login</a><a className="button button-secondary" href="tel:+15704215887">Call the shop</a></div></div></div></section>;

  const chooseService = (value: BarberServiceOption) => { setServiceId(value.id); setSelectedBarberId(requestedBarber || 'any'); setDateKey(''); setSlot(null); setCandidateBarbers([]); setWaitlist(false); setWeekStart(0); setStep('barber'); };
  const chooseBarber = (value: string) => { setSelectedBarberId(value); setDateKey(''); setSlot(null); setCandidateBarbers([]); setWaitlist(false); setWeekStart(0); setStep('schedule'); };
  const chooseDate = (value: string) => { setDateKey(value); setSlot(null); setCandidateBarbers([]); setWaitlist(false); setError(''); };

  const continueWithSlot = (value: TimeSlot) => {
    if (!service || !dateKey) return;
    const candidates = availableBarbersForSlot({ service, requestedBarberId: selectedBarberId, date: dateKey, slot: value, profiles: staffProfiles, appointments, excludeAppointmentId: editingAppointment?.id });
    if (!candidates.length) { setError('That opening was just taken. Choose another time.'); setAppointments(readAppointments()); return; }
    setSlot(value);
    setWaitlist(false);
  ...
  const selectCandidate = (barber: BarberDirectoryEntry) => { setSelectedBarberId(barberId(barber)); setCandidateBarbers([]); setStep('details'); };
  const continueWithWaitlist = () => { setSlot(null); setCandidateBarbers([]); setWaitlist(true); setStep('details'); };
  const back = () => { if (step === 'service') window.location.assign('/book'); else if (step === 'barber') setStep('service'); else if (step === 'schedule') editingAppointment ? window.location.assign('/account') : setStep('barber'); else if (step === 'details') setStep('schedule'); else window.location.assign('/account'); };
  ...
  const submit = () => {
    ...
    const resolvedBarber = selectedBarber ?? (slot ? availableBarbersForSlot({ service, requestedBarberId: selectedBarberId, date: dateKey, slot, profiles: staffProfiles, appointments, excludeAppointmentId: editingAppointment?.id })[0] ?? null : null);
    if (!waitlist && !resolvedBarber) { setError('That opening is no longer available. Choose another time.'); setStep('schedule'); return; }
    ...
    const shared = { requestedBarberId: selectedBarberId, assignedBarberId: resolvedId, barberName: resolvedBarber?.name ?? 'Any available barber', date: dateKey, time, startMinutes, endMinutes: startMinutes + service.durationMinutes, proposedDate: null, proposedTime: null, proposedStartMinutes: null, customerName: customer.name.trim(), customerEmail: customer.email.trim().toLowerCase(), customerPhone: normalizePhone(customer.phone), customerNote: customer.note.trim() };
    const appointment = editingAppointment ? updateAppointment(editingAppointment.id, { ...shared, status: waitlist ? 'waitlisted' : 'requested', clientResponse: null, source: waitlist ? 'walk-in' : 'website' }) : createAppointmentRequest({ serviceId: service.id, serviceName: service.name, price: service.price, priceCents: service.priceCents, durationMinutes: service.durationMinutes, ...shared, phoneVerified: Boolean(profile?.phoneVerified), accountPreference: account ? 'account' : 'guest', source: waitlist ? 'walk-in' : 'website', staffNote: '' });
  ...
  return <section className="section booking-v2-page booking-v4-page booking-v5-page platform-pattern platform-pattern-booking"><div className="container route-wide"><header className="booking-v2-header booking-v4-header"><div><p className="eyebrow">{editingAppointment ? 'Change appointment' : 'Barber booking'}</p><h1>{editingAppointment ? 'Choose a new opening.' : 'Request your chair.'}</h1></div></header><Progress step={step} />
    {step === 'service' ? <section className="booking-v2-panel booking-v4-panel"><PanelToolbar onBack={back} showLoctician={!editingAppointment} /><div className="booking-v2-panel-heading"><h2>Choose a service</h2></div><div className="booking-v2-service-list">{barberServiceOptions.map((item) => <button type="button" key={item.id} onClick={() => chooseService(item)}><span><small>{item.category}</small><strong>{item.name}</strong></span><span><strong>{item.price}</strong><small>{item.durationMinutes} min</small></span></button>)}</div></section> : null}
    {step === 'barber' && service ? <section className="booking-v2-panel booking-v4-panel"><PanelToolbar onBack={back} showLoctician={!editingAppointment} /><div className="booking-v2-panel-heading"><h2>Choose your barber</h2><p>{service.name} · {service.price} · {service.durationMinutes} minutes</p></div><div className="booking-v2-barber-grid"><button type="button" onClick={() => chooseBarber('any')}><strong>Any available barber</strong><span>Compare every eligible chair for the selected opening.</span></button>{barbers.map((barber) => <button type="button" key={barberId(barber)} onClick={() => chooseBarber(barberId(barber))}><strong>{barber.name.replace(/\.$/, '')}</strong><span>{barber.profile ? 'Uses published availability' : 'Uses current shop hours'}</span></button>)}</div></section> : null}
    {step === 'schedule' && service ? <section className="booking-v2-panel booking-v4-panel booking-v4-schedule"><PanelToolbar onBack={back} showLoctician={!editingAppointment} /><div className="booking-v2-panel-heading"><h2>Choose a date and time</h2><p>{service.name} with {selectedBarber?.name.replace(/\.$/, '') ?? 'any available barber'}.</p></div><div className="booking-v4-calendar"><div className="booking-v4-calendar-toolbar"><button type="button" disabled={weekStart === 0} onClick={() => setWeekStart((value) => Math.max(0, value - 7))} aria-label="Previous week">←</button><strong>{formatWeekRange(visibleDays)}</strong><button type="button" disabled={weekStart + 7 >= calendarDays.length} onClick={() => setWeekStart((value) => Math.min(calendarDays.length - 7, value + 7))} aria-label="Next week">→</button></div><div className="booking-v4-week" role="group" aria-label={formatWeekRange(visibleDays)}>{visibleDays.map((day) => { const openings = day.isClosed ? [] : slotsForDate(day.key); const status = day.isClosed ? 'Closed' : openings.length ? 'Open' : 'Full'; return <button className={dateKey === day.key ? 'is-selected' : ''} type="button" key={day.key} disabled={day.isClosed} aria-pressed={dateKey === day.key} onClick={() => chooseDate(day.key)}><small>{day.isToday ? 'Today' : day.weekday}</small><strong>{day.day}</strong><span>{day.month}</span><em>{status}</em></button>; })}</div></div>{dateKey ? <div className="booking-v2-times booking-v4-times"><h3>{formatLongDate(dateKey)}</h3>{availableSlots.length ? <div>{availableSlots.map((value) => <button type="button" key={value.startMinutes} onClick={() => continueWithSlot(value)}>{value.label}</button>)}</div> : <div className="booking-v2-no-times"><strong>No openings match this selection.</strong><p>{sameDayFull ? 'Today is full for the service and barber you selected.' : 'Choose another day or barber to continue.'}</p>{sameDayFull ? <button className="button" type="button" onClick={continueWithWaitlist}>Join today’s waitlist</button> : null}</div>}</div> : <p className="booking-v2-prompt">Choose a day to view available appointment times.</p>}{candidateBarbers.length && slot ? <div className="booking-v5-candidate-picker"><p className="eyebrow">Multiple chairs are open at {slot.label}</p><h3>Choose the barber for this opening</h3><div>{candidateBarbers.map((barber) => <button type="button" key={barberId(barber)} onClick={() => selectCandidate(barber)}><strong>{barber.name.replace(/\.$/, '')}</strong><span>{slot.label}</span></button>)}</div></div> : null}{error ? <p className="form-error" role="alert">{error}</p> : null}</section> : null}
    {step === 'details' && service && dateKey ? <section className="booking-v2-panel booking-v4-panel"><PanelToolbar onBack={back} showLoctician={!editingAppointment} /><div className="booking-v2-panel-heading"><h2>{waitlist ? 'Join today’s waitlist' : 'Review your appointment'}</h2><p>{editingAppointment ? 'Send the new opening to the shop for approval.' : account ? 'Your account information was filled in automatically.' : 'An account is not required.'}</p></div><div className="booking-v2-review"><div><small>Service</small><strong>{service.name}</strong></div><div><small>Barber</small><strong>{selectedBarber?.name.replace(/\.$/, '') ?? 'Any available barber'}</strong></div><div><small>Date</small><strong>{formatLongDate(dateKey)}</strong></div><div><small>{waitlist ? 'Status' : 'Time'}</small><strong>{waitlist ? 'Waiting for an opening' : slot?.label}</strong></div><div><small>Price</small><strong>{service.price}</strong></div></div><div className="booking-v2-fields booking-v4-fields"><label><span>Name</span><input autoComplete="name" value={customer.name} onChange={(event) => setCustomer({ ...customer, name: event.target.value })} /></label><label><span>Email</span><input type="email" autoComplete="email" value={customer.email} onChange={(event) => setCustomer({ ...customer, email: event.target.value })} /></label><label><span>Mobile phone</span><input type="tel" autoComplete="tel" value={customer.phone} onChange={(event) => setCustomer({ ...customer, phone: event.target.value })} /></label><label className="booking-v2-wide"><span>Note for the barber <small>Optional</small></span><textarea rows={4} value={customer.note} onChange={(event) => setCustomer({ ...customer, note: event.target.value })} /></label></div>{error ? <p className="form-error" role="alert">{error}</p> : null}<p className="booking-v2-disclaimer">Submitting sends a request to the shop. It is not confirmed until the barber or shop approves it.</p><div className="booking-v2-submit-row"><button className="button" type="button" onClick={submit}>{editingAppointment ? 'Request appointment change' : waitlist ? 'Join today’s waitlist' : 'Request appointment'}</button></div></section> : null}
    {step === 'complete' && confirmation ? <section className="booking-v2-confirmation booking-v5-confirmation"><p className="eyebrow">Request received</p><h2>{editingAppointment ? 'Your change request was sent.' : confirmation.status === 'waitlisted' ? 'You joined today’s waitlist.' : 'Your appointment request was sent.'}</h2><p>{editingAppointment ? 'Your existing appointment date and time remain in place until the shop approves this pending change.' : confirmation.status === 'waitlisted' ? 'A barber can claim the request when an opening becomes available.' : 'This request remains pending until the shop confirms it.'}</p><dl><div><dt>Service</dt><dd>{confirmation.serviceName}</dd></div><div><dt>Barber</dt><dd>{confirmation.barberName.replace(/\.$/, '')}</dd></div><div><dt>Date</dt><dd>{formatLongDate(confirmation.date)}</dd></div><div><dt>Time</dt><dd>{confirmation.time}</dd></div></dl><div className="booking-v2-complete-actions"><a className="button" href="/account">Manage appointment</a><a className="button button-secondary" href={`/book?appointment=${confirmation.id}`}>Edit request</a><button className="text-button danger" type="button" onClick={cancelConfirmation}>Cancel request</button></div></section> : null}
  </div></section>;
}
