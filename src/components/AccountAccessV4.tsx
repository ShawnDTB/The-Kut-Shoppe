...
export function AccountAccessV4() {
  ...
  if (sessionAccount) return <RoleDashboardV4 />;
  ...
  return (
    <section className="section v2-auth-page v4-auth-page platform-pattern platform-pattern-account">
      <div className="container v4-auth-container">
        <div className="v2-auth-card v4-auth-card">
          ...
          <form className="v2-auth-form v4-auth-form" onSubmit={(event) => { event.preventDefault(); void (mode === 'sign-in' ? signIn() : create()); }}>
            <p className="v2-auth-mode-note">{mode === 'sign-in' ? 'Use the email and password attached to your account.' : 'Create a customer account with the essentials.'}</p>
            ...
            {mode === 'sign-in' ? <button className="text-button" type="button" disabled>Forgot password will use verified email in production</button> : <p className="v2-auth-note">Phone and shipping details are requested only when needed.</p>}
          </form>
