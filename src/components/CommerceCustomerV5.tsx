  ...
  useEffect(() => {
    ...
    void loader.then(() => {
      ...
      autocomplete.addListener('place_changed', () => {
        ...
        onChange({ ...value, line1: `${component('street_number')} ${component('route')}`.trim(), city: component('locality') || component('postal_town') || component('sublocality'), state: component('administrative_area_level_1', true), postalCode: component('postal_code') });
      });
    }).catch(() => undefined);
    return () => { active = false; };
  }, [onChange, value]);
  return <label className="catalog-form-wide">Street address <span aria-hidden="true">*</span><input ref={inputRef} required autoComplete="street-address" placeholder="Start typing a street address" aria-invalid={Boolean(error)} value={value.line1} onChange={(event) => onChange({ ...value, line1: event.target.value })} />{error ? <small className="field-error">{error}</small> : null}</label>;
}
...
export function ProductDetailPageV5({ slug }: { slug: string }) {
  ...
  return <section className="section commerce-product-page commerce-product-page-v5 platform-pattern platform-pattern-products"><div className="container route-wide"><div className="commerce-product-v5-topline"><a className="text-link" href="/shop">← Back to Shop</a><span>{product.category}</span></div>{notice ? <p className="commerce-action-notice" role="status">✓ {notice}</p> : null}<div className="commerce-product-detail commerce-product-detail-v5"><div className="commerce-product-gallery"><div className="commerce-product-primary-image"><ProductImage product={product} variant={selectedVariant} /></div></div><div className="commerce-product-detail-copy"><h1>{product.name}</h1>{selectedVariant ? <div className="commerce-product-purchase commerce-product-purchase-v5"><div><strong>{formatMoney(selectedVariant.priceCents)}</strong><small>{available > 0 ? `${available} available` : 'Out of stock'}</small></div><button className="button" type="button" disabled={available <= 0} onClick={add}>Add to cart</button></div> : null}{activeVariants.length > 1 ? <fieldset className="commerce-variant-picker"><legend>Choose an option</legend>{activeVariants.map((variant) => <label className={selectedVariant?.id === variant.id ? 'is-selected' : ''} key={variant.id}><input type="radio" name="variant" checked={selectedVariant?.id === variant.id} onChange={() => { setSelectedVariantId(variant.id); setNotice(''); }} /><span><strong>{variant.name}</strong><small>{formatMoney(variant.priceCents)} · {getAvailableStock(product, variant)} available</small></span></label>)}</fieldset> : null}<p className="lede commerce-product-description-v5">{product.description}</p><div className="commerce-product-fulfillment">{product.pickupEnabled ? <span>Pickup at 518 Main Street</span> : null}{product.shippingEnabled ? <span>Shipping available</span> : null}</div>{product.amazonUrl ? <a className="button button-secondary" href={product.amazonUrl} target="_blank" rel="noopener noreferrer">Buy on Amazon <span aria-hidden="true">↗</span></a> : null}</div></div></div><CartDrawerV5 open={drawerOpen} products={products} cart={cart} onClose={() => setDrawerOpen(false)} onCartChange={setCart} /></section>;
}

function ReceiptV5({ order, products }: { order: StoreOrder; products: StoreProduct[] }) {
  const shipping = order.fulfillment === 'shipping';
  return <section className="commerce-order-confirmation commerce-receipt-v5" aria-labelledby="receipt-heading"><div className="commerce-receipt-header"><div><p className="eyebrow">Order request receipt</p><h1 id="receipt-heading">{shipping ? 'Shipping Request Received' : 'Pickup Request Received'}</h1><p>{shipping ? 'The shop will confirm inventory, shipping rate, tax, payment, and delivery timing before accepting this order.' : 'The shop will review the request and send an update when the order is accepted and ready for pickup.'}</p></div><button className="button button-secondary print-hidden" type="button" onClick={() => window.print()}>Print or save as PDF</button></div><dl className="commerce-receipt-meta"><div><dt>Reference</dt><dd>{order.id}</dd></div><div><dt>Status</dt><dd>{order.status.replaceAll('-', ' ')}</dd></div><div><dt>Method</dt><dd>{shipping ? 'Shipping' : 'Pickup at 518 Main Street'}</dd></div><div><dt>Estimated timing</dt><dd>{shipping ? 'Provided after carrier and rate confirmation' : 'Ready time provided after shop acceptance'}</dd></div></dl><div className="commerce-receipt-lines">{order.items.map((item) => { const product = products.find((entry) => entry.id === item.productId); const variant = product?.variants.find((entry) => entry.id === item.variantId); return <article key={`${item.productId}-${item.variantId}`}>{product ? <div className="commerce-receipt-image"><ProductImage product={product} variant={variant} /></div> : null}<div><strong>{item.name}</strong><span>{item.variantName} · Quantity {item.quantity}</span></div><strong>{formatMoney(item.unitPriceCents * item.quantity)}</strong></article>; })}</div>{shipping && order.shippingAddress ? <address><strong>Ship to</strong><span>{order.customer.name}</span><span>{order.shippingAddress.line1}</span>{order.shippingAddress.line2 ? <span>{order.shippingAddress.line2}</span> : null}<span>{order.shippingAddress.city}, {order.shippingAddress.state} {order.shippingAddress.postalCode}</span></address> : <address><strong>Pickup location</strong><span>The Kut Shoppe</span><span>518 Main Street</span><span>Stroudsburg, PA 18360</span></address>}<dl className="commerce-receipt-totals"><div><dt>Subtotal</dt><dd>{formatMoney(order.subtotalCents)}</dd></div><div><dt>Shipping and tax</dt><dd>{shipping ? 'Pending confirmation' : 'Not included'}</dd></div></dl><p className="fine-print">This is an order-request receipt, not proof of payment. A paid receipt or invoice is issued only after payment is completed.</p><div className="commerce-inline-actions print-hidden"><a className="button" href="/account">View order in Account</a><a className="button button-secondary" href="/shop">Continue shopping</a></div></section>;
}
