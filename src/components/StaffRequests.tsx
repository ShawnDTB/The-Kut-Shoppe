...
function RequestDetail({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  const [action, setAction] = useState<'confirm' | 'decline' | 'cancel' | 'keep' | null>(null);
  ...
  return <section className="customer-record-detail" aria-busy={working}>
    ...
    {request ? <><p className="customer-status">{request.status.replaceAll('_', ' ')}</p><h3>{request.customerName} · {request.serviceName}</h3>
      ...
      {request.status === 'requested' || (request.status === 'confirmed' && request.cancellationState === 'pending') ? action ? <form className="customer-form customer-security-section" onSubmit={(event) => void decide(event)}>
        ...
        <div className="customer-form-actions"><button className="button" disabled={working}>{working ? 'Saving…' : action === 'cancel' ? 'Approve cancellation' : action === 'keep' ? 'Decline cancellation' : action === 'confirm' ? 'Confirm appointment' : 'Decline request'}</button><button className="button button-secondary" type="button" disabled={working} onClick={refresh}>Refresh before changing decision</button></div>
      </form> : request.cancellationState === 'pending' ? <div className="customer-form-actions"><button className="button" disabled={working} onClick={() => setAction('cancel')}>Review cancellation approval</button><button className="button button-secondary" disabled={working} onClick={() => setAction('keep')}>Review keeping appointment</button></div> : <div className="customer-form-actions"><button className="button" disabled={working || !request.startsAt || !request.endsAt} onClick={() => setAction('confirm')}>Review confirmation</button><button className="button button-secondary" disabled={working} onClick={() => setAction('decline')}>Review decline</button></div> : <p>This request has been resolved. Refresh the queue to continue.</p>}
    </> : null}
