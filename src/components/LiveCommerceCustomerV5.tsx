import { AccountApiError } from '../data/customer-api';
import { useEffect, useRef, useState } from 'react';
import { createPortal } from 'react-dom';
import { getPlatformSessionAccount, isValidEmail } from '../data/commerce-identity';
import { isValidPhone } from '../data/commerce-identity';
import { getCustomerProfileV5 } from '../data/commerce-identity';
import {
  addToCart,
  createOrder,
  formatMoney,
...
export function ProductDetailPageV5({ slug }: { slug: string }) {
  ...
  const add = () => { if (!selectedVariant || available <= 0) return; let next; try { next = addToCart(product.id, selectedVariant.id, 1); } catch(error) { setNotice(error instanceof Error?error.message:'Unable to add this item.'); return; } setCart(next); setNotice(`${product.name}${selectedVariant.name !== 'Default' ? ` · ${selectedVariant.name}` : ''} added to your cart.`); setDrawerOpen(true); };
...
export function CheckoutPageV5({onSubmitted = (id:string)=>window.location.assign(`/account?view=orders&record=${encodeURIComponent(id)}`)}: {onSubmitted?:(id:string)=>void}) {
  ...
  const pending = useRef<{ fingerprint: string; key: string; uncertain: boolean } | null>(null);
  useEffect(() => subscribeToStorefrontChanges(() => { setProducts(readProducts()); setCart(readCart()); }), []);
  const lines = resolveLines(cart, products);
  const subtotal = lines.reduce((total, line) => total + line.variant.priceCents * line.item.quantity, 0);
