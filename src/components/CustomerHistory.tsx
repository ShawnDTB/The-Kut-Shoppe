...
export function CustomerHistory({ kind }: { kind: 'appointments' | 'orders' }) {
  ...
  return <section aria-busy={working}>
    ...
    {items?.length ? <ul className="customer-records">{items.map((item) => <li key={item.id}>{'serviceName' in item ? <>
      ...
      <a href={business.phoneHref}>Call about this appointment</a>
    </> : <><div><h3>Order {item.id.slice(-8)}</h3><p>{statusLabel(item.status)} · {statusLabel(item.fulfillment)} · {time(item.createdAt)}</p></div><strong>{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(item.totalCents / 100)}</strong></>}</li>)}</ul> : items && !working && !error ? <div className="customer-empty">
