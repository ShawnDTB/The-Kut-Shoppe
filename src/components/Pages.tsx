...
function PrivacyRoute() {
  return (
    <article className="legal-page route-content">
      ...
      <section><h2>Account information</h2><p>When account access is enabled, account details are stored on the server. Passwords are stored as one-way hashes. Email verification and password recovery use expiring codes. Changing a sign-in email requires your current password and confirmation codes from both the current and new inboxes. Pending email-change records temporarily hold both addresses; completing the change signs out all devices. Optional phone numbers are contact details and are not used to verify account ownership.</p></section>
      <section><h2>Cookies and local storage</h2><p>Account access uses an essential session cookie to keep you signed in. Signing out ends that session; account security also lets you sign out on every device. This website does not currently run optional advertising or audience analytics. Any future optional tracking will be described here and assessed before it is enabled.</p></section>
      <section><h2>Account service providers</h2><p>When accounts are enabled, Cloudflare hosts the account service and database and provides Turnstile abuse protection. Resend delivers verification, password recovery, and email-change confirmation emails. Booking through Booksy or Crowned by Steph continues under the selected provider’s privacy notice; those appointments are not automatically imported into your account.</p></section>
