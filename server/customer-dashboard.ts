...
import type { CustomerDashboard } from '../src/shared/dashboard';
import type { CustomerAppointment, CustomerOrder } from '../src/shared/customer';
...
export async function customerDashboard(env: Env, userId: string): Promise<CustomerDashboard> {
  ...
  const result = await env.DB.batch<{ results: Record<string, unknown>[] }>([
    env.DB.prepare(`SELECT COALESCE(SUM(${upcoming}),0) AS upcoming,
      COALESCE(SUM(status IN ('requested','waitlisted')),0) AS pending,
    ...
    env.DB.prepare(`${appointment} WHERE a.customer_user_id=? ORDER BY a.created_at DESC,a.id DESC LIMIT 4`).bind(userId),
    env.DB.prepare('SELECT id,status,fulfillment_type AS fulfillment,total_cents AS totalCents,created_at AS createdAt FROM orders WHERE customer_user_id=? ORDER BY created_at DESC,id DESC LIMIT 3').bind(userId),
  ]);
  return { counts: result[0]!.results[0] as unknown as CustomerDashboard['counts'],
    nextVisit: (result[1]!.results[0] ?? null) as CustomerAppointment | null,
    recentAppointments: result[2]!.results as unknown as CustomerAppointment[],
