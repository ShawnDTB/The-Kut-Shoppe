...
async function route(request: Request, env: Env): Promise<Response> {
  ...
  if (action === 'POST /api/v1/auth/verify' || action === 'POST /api/v1/auth/reset') {
    ...
    const userId = await consumeChallenge(env, id, code, reset ? 'reset_password' : 'verify_email', passwordHash);
    ...
    return json({ account: await getAccount(env, userId) }, 200, await createSession(env, userId));
  }
